<template>
  <div class="flex">
        ...
        >
          ...
          <template #title="{ dataRef }">
            <span class="text-center">
              {{
                dataRef.title.length > 10
                  ? dataRef.title.slice(0, 10) + "..."
                  : dataRef.title
              }}
            </span>
          </template>
    ...
    <div
      class="flex-1 b-r-solid shadow-[0_0_10px_0_rgba(50,50,50,0.2)] h-100vh"
    >
      <a-tabs
        ...
        <template #rightExtra>
          <div class="flex items-center justify-center">
            ...
            <a-popover title="点击下载二维码">
              <a-button v-if="ispublic" class="m-1"> 分享 </a-button>
              <template #content>
...
const onSelect: TreeProps["onSelect"] = (selectedKeys, info) => {
  if (
    !info.node.children &&
    info.selected &&
    panes.value.find((item) => {
      return item.key === (selectedKeys[0] as string);
    }) === undefined
  ) {
    panes.value.push({
      title: info.node.title,
      content: "",
      key: selectedKeys[0] as string,
    });
  ...
  (newVal) => {
    if (newVal !== "") {
      ...
      if (!isSelect) {
        ...
        noteStore.markdownId = "";
        console.log(noteStore.markdownId, newVal, "edit.vue");
      } else {
        message.error("文章已打开");
